    ...
    try {
      ...
      switch (widget.serviceType) {
        ...
        // For trainer, we'll use the existing data since there's no separate details API
          setState(() {
            serviceDetails = ServiceDetailsModel.fromJson({
              'id': widget.service.id,
              'name': widget.service.name,
              'trainer': widget.service.provider,
              'rating': widget.service.rating,
              'open_time': widget.service.time.split('-')[0].trim(),
              'close_time': widget.service.time.split('-').length > 1
                  ? widget.service.time.split('-')[1].trim()
                  : '06:00 PM',
              'fee': widget.service.price.replaceAll('₹', '').replaceAll('.00', ''),
              'city': widget.service.location,
