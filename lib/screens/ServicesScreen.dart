...
class ServiceListPage extends StatefulWidget {
  final int serviceType; // 1: Foster, 2: Groomer, 3: Trainer, 4: Vet, 5: Shelter
...
class _ServiceListPageState extends State<ServiceListPage> {
  ...
  void initState() {
    super.initState();
    if (widget.serviceType == 1 ||widget.serviceType == 2 || widget.serviceType == 3 || widget.serviceType == 4) {
      // Groomers, trainers, and shelters need location
      _getCurrentLocation();
    } else {
      // Foster homes and vets don't need location initially
      fetchServices();
    }
  ...
  Future<void> _loadFilters() async {
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          'type': widget.serviceType.toString(),
        }),
      );
  ...
  void _showFilterOptions() {
    if (filterGroups.isEmpty) {
      _showMessage('No filters available for this service', isError: false);
      return;
    }
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final groupFilters = filters
                                .where((filter) => filter.filterGroupId == group.id)
                                .toList();
                            final selectedCount = groupFilters
                                .where((filter) => filter.isSelected)
                                .length;
                            final isSelected = _selectedFilterGroupIndex == index;
                          ...
                          child: _selectedFilterGroupIndex < filterGroups.length
                            ...
                            children: [
                              ...
                              Padding(
                                ...
                                child: Text(
                                  filterGroups[_selectedFilterGroupIndex].name,
                                  style: const TextStyle(
                                    color: Colors.black,
                                    fontSize: 16,
                                    fontWeight: FontWeight.w600,
                              ...
                              ...filters
                                  .where((filter) =>
                              filter.filterGroupId == filterGroups[_selectedFilterGroupIndex].id)
  ...
  void _showSortOptions() {
    ...
    // Reset to first group when opening filters
    _selectedFilterGroupIndex = 0;
  ...
  void _applyFilters() {
    final selectedFilters = filters.where((filter) => filter.isSelected).toList();
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (widget.serviceType == 1) {
          ...
          // Groomers
          serviceList = data['listgrooming'] ?? data['groomers'] ?? data['data'] ?? [];
        } else if (widget.serviceType == 4) {
          // Trainers
          serviceList = data['listtrainer'] ?? [];
        } else if (widget.serviceType == 5) {
          // Foster homes - data is a list directly
          serviceList = data is List ? data : [];
        } else {
          serviceList = [];
        }
        ...
        setState(() {
          errorMessage = 'Failed to load ${serviceTitle.toLowerCase()}: ${response.statusCode}';
          isLoading = false;
        });
      }
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
            ...
            child: Row(
              children: [
                ...
                if (!isLoading && !_isLoadingLocation && errorMessage == null)
                  IconButton(
                    onPressed: () {
                      if (widget.serviceType == 1 || widget.serviceType == 5 ||
                          (_latitude != 0.0 && _longitude != 0.0)) {
                        fetchServices();
                      } else {
                        _getCurrentLocation();
          ...
          if ((widget.serviceType == 2 || widget.serviceType == 3 || widget.serviceType == 4) &&
            ...
            Padding(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    Icon(
                      _locationError.isNotEmpty ? Icons.error_outline : Icons.location_on,
                      color: _locationError.isNotEmpty ?
                      const Color(0xFFEF4444) :
                      const Color(0xFF60A5FA),
                      size: 16,
  ...
  Widget _buildContent() {
    ...
    if (isLoading && (widget.serviceType == 1 || widget.serviceType == 5 || _latitude != 0.0)) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
    ...
    if (services.isEmpty && (widget.serviceType == 1 || widget.serviceType == 5 || _latitude != 0.0)) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
    ...
    if ((widget.serviceType == 1 || widget.serviceType == 3 || widget.serviceType == 4) &&
        _latitude == 0.0 && !_isLoadingLocation) {
      return const Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
