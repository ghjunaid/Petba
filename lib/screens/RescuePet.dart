...
class _RescuePageState extends State<RescuePage> {
  ...
  void initState() {
    ...
    if (widget.customerId.isEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _showMessage(
            'Invalid user session. Please login again.', isError: true);
      ...
      try {
        ...
        setState(() {
          ...
          if (placemarks.isNotEmpty) {
            ...
            '$street${street.isNotEmpty && subLocality.isNotEmpty
                ? ', '
                : ''}$subLocality';
  ...
  Future<void> _loadRescuePets({bool refresh = false}) async {
    ...
    try {
      ...
      // Parse customerId to int, fallback to default if parse fails
      int customerIdInt;
      try {
        customerIdInt = int.parse(widget.customerId);
      } catch (e) {
        customerIdInt = 2; // Default fallback
        print('Warning: Could not parse customerId, using default: $customerIdInt');
      }
      ...
      final requestBody = {
        ...
        'filter': {
          ...
          'animalType': [],
          'gender': [],
          'city': [73318], // Add the city filter from your Postman request
        },
        'sort': _sortOption,
      };
      ...
      if (response.statusCode == 200) {
        ...
        final List<Map<String, dynamic>> newRescues =
            ...
            [
                ...
                child: Row(
                  children: [
                    Icon(Icons.location_on,
                        color: _sortOption == '1' ? Color(0xFFFF6B6B) : Colors
                            .white,
                        size: 20),
                    SizedBox(width: 8),
              ...
              PopupMenuItem(
                ...
                child: Row(
                  children: [
                    Icon(Icons.schedule,
                        color: _sortOption == '2' ? Color(0xFFFF6B6B) : Colors
                            .white,
                        size: 20),
                    SizedBox(width: 8),
                    Text('Sort by Recent',
  ...
  Widget _buildRescueCard(Map<String, dynamic> rescue) {
    ...
    double distance = double.tryParse(rescue['Distance']?.toString() ?? '0') ?? 0;
            ...
            child: imageUrl.isNotEmpty
              ...
              child: Image.network(
                ...
                errorBuilder: (context, error, stackTrace) {
                  print('Image loading error for $imageUrl: $error'); // Debug log
                  return Container(
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Container(
                      padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                      decoration: BoxDecoration(
                        color: status == '0' ? Colors.orange[600] : Colors.green[600],
                        borderRadius: BorderRadius.circular(12),
                      ),
                      child: Text(
                ...
                if (rescue['description'] != null && rescue['description']
                    .toString()
                    .isNotEmpty) ...[
                  SizedBox(height: 12),
                  Text(
                ...
                SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Details
                      _buildDetailRow(
                          'Condition', rescue['ConditionType'] ?? 'Unknown'),
                      _buildDetailRow('Severity', _getConditionText(
                          rescue['conditionLevel_id']?.toString() ?? '0')),
                      _buildDetailRow('Status',
                          rescue['status'] == '0' ? 'Needs Rescue' : 'Rescued'),
                      _buildDetailRow(
                          'Location', rescue['address'] ?? 'Not available'),
                      _buildDetailRow('City', rescue['city'] ?? 'Unknown'),
                      _buildDetailRow('Distance', '${double
                          .tryParse(
                          rescue['Distance']?.toString() ?? '0')
                          ?.toStringAsFixed(1) ?? '0'} km'),
